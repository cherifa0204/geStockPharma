...
@php
    ...
    $title = ($achat ?? null) != null ? "Modification d'un" : "Enregistrement d'un nouveau";
@endphp
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
            <h1 class="text-3xl font-bold tracking-tight text-slate-900">{{ $title }} achat</h1>
            <p class="text-sm text-slate-500 mt-1">Saisissez les réapprovisionnements de stock commandés auprès de vos fournisseurs.</p>
        </div>
        
        <div>
            <a href="{{ route('produits.create') }}" class="inline-flex items-center space-x-2 rounded-xl bg-white border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 transition-all">
    ...
    <!-- Main Form -->
    <form action="{{ route('achats.'.$name, $achat ?? '') }}" method="post" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        @if ($achat ?? null)
            @method("PUT")
        @endif
        @csrf

        <!-- Saisie Produit & Quantité (Left Column) -->
        <div class="lg:col-span-1 space-y-6">
            <div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm space-y-4">
                <h2 class="font-bold text-slate-800 text-base">Ajouter un produit à l'achat</h2>
                ...
                <div class="space-y-2">
                    ...
                    <input id="id_prd" type="text" hidden>
                </div>

                <!-- Quantity input -->
                <div class="space-y-2">
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-400">Quantité achetée</label>
        ...
        <!-- Table of Purchase Lines (Right Columns) -->
        <div class="lg:col-span-2 space-y-6">
            <div class="rounded-2xl border border-slate-100 bg-white shadow-sm overflow-hidden">
                <div class="p-6 border-b border-slate-50 bg-slate-50/50">
                    <h2 class="font-bold text-slate-800 text-lg">Liste des lignes d'achat</h2>
                </div>

                <div class="overflow-x-auto">
                    <table id="lignes_achat" class="w-full text-sm text-left text-slate-500">
                        <thead class="text-xs text-slate-400 uppercase bg-slate-50/50 border-b border-slate-100">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-4 w-32">Quantité achetée</th>
                        ...
                        <tbody id="id_tbody" class="divide-y divide-slate-100">
                            ...
                            <tr id="{{ $loop->index+1 }}" class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <input class="w-20 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-center text-sm font-semibold text-slate-700 outline-none transition-all focus:border-teal-500 focus:bg-white" type="number" name="quantite[]" value="{{ $ligne->quantite_achat }}" >
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end space-x-2">
                                        <input type="button" value="Editer" onclick="editer_ligne('{{ $loop->index+1 }}')" class="inline-flex items-center rounded-xl bg-teal-50 px-3 py-1.5 text-xs font-semibold text-teal-700 hover:bg-teal-100 cursor-pointer transition-colors">
                                        <input type="button" value="Supprimer" onclick="supprimer_ligne('{{ $loop->index+1 }}')" class="inline-flex items-center rounded-xl bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-100 cursor-pointer transition-colors">
                                    </div>
                                </td>
                            </tr>
                            @endforeach
...
<script type="text/javascript">
    ...
    produits_ul.addEventListener('click', function(event) {
        if (event.target.tagName === 'LI' && event.target.id) {
            ...
            input_id.value = event.target.id;
            productsContainer.style.display = 'none';
            clear_msg_prd();
        }
    });
    ...
    function ligne_html(num_ligne, id_ligne_achat, id_produit, nom_produit, quantite_achat) {
        return `<tr class='hover:bg-slate-50/50 transition-colors' id='${num_ligne}'>` +
                ...
                `</td>` +
                `<td class='px-6 py-4'>` +
                    `<input class='w-20 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-center text-sm font-semibold text-slate-700 outline-none transition-all focus:border-teal-500 focus:bg-white' type='number' name='quantite[]' value='${quantite_achat}'>` +
                `</td>` +
                `<td class='px-6 py-4 text-right'>` +
                    `<div class='flex items-center justify-end space-x-2'>` +
    ...
    ajouter_balise.addEventListener('click', function(e) {
        ...
        if (!liste_prod.includes(searchInput.value)) {
            msg_prd.innerText = "Le produit '" + searchInput.value + "' n'existe pas.";
            return;
        }
        clear_msg_prd();
        
        let nom_produit = searchInput.value;
        let produit_id = recuperer_id(input_id.value);
        let quantite = parseInt(quantite_input.value);
        
        if (isNaN(quantite) || quantite <= 0) {
            msg.innerText = "La quantité d'achat doit être supérieure à 0";
            return;
        }
    ...
    function editer_ligne(id_ligne) {
        ...
        input_id.value = "pro_" + ligne.children[1].children[1].value;
